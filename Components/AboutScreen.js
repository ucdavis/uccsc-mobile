...
export default class AboutScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = { strokeOffset: new Animated.Value(870) };

    this.state.strokeOffset.addListener(r => {
      this._myPath.setNativeProps({ r: r.value.toString() });
    });
  }

  _animate = () => {
    console.log(this._mysvg);
    // make the radius 100 after 2 seconds
    Animated.timing(this.state.strokeOffset, {
      duration: 1000 * 2,
      toValue: 0,
    }).start();
  };
  ...
  render() {
    const fill = "#f1c40f";
    let AnimatedPath = Animated.createAnimatedComponent(Svg.Path);
    return (
      <View style={styles.container}>
        <Button onPress={this._animate} title="Animate" />
        <Text style={styles.sectionText}>About Screen</Text>
        <View>
          <Svg height={250} width={500} ref={ r => this._mysvg = r }>
            <AnimatedPath
              ref={r => (this._myPath = r)}
              id="hood"
              fill="none"
              stroke="white"
              strokeWidth={1}
