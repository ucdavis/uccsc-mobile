...
export default (rootReducer, rootSaga) => {
  ...
  const persistedRootReducer = ReduxPersistConfig.active
    ? persistReducer(ReduxPersistConfig.storeConfig, rootReducer)
    : rootReducer;

  const store = createAppropriateStore(persistedRootReducer, compose(...enhancers));

  // configure persistStore and check reducer version number
  const persistor = ReduxPersistConfig.active
    ? persistStore(store)
    : null;
